...
public interface AssetAllocationRepo extends JpaRepository<AssetAllocationVO, Long> {
	...
	@Query(nativeQuery = true, value = "WITH cal AS (\r\n"
			...
			+ "      AND a.branchcode=?2\r\n"
			+ "      AND a.assetstatus='A'\r\n"
			+ "      AND a.sourcescreencode='AL'\r\n"
			+ "      AND a.astatus='A'\r\n"
			+ "      AND a.qty = 1\r\n"
			+ "      AND NOT EXISTS (\r\n"
			+ "            SELECT 1\r\n"
			+ "            FROM assetstock r\r\n"
			+ "            WHERE r.sourcescreencode='AR'\r\n"
			+ "              AND r.assetcode = a.assetcode\r\n"
			+ "              AND r.serialnumber = a.serialnumber\r\n"
			+ "      )\r\n"
			+ ")\r\n"
			+ "SELECT\r\n"
			+ "    SUM(totalcount) AS totalcount,\r\n"
			+ "    SUM(allocated) AS allocated,\r\n"
			+ "    (SUM(totalcount) - SUM(allocated)) AS avalible\r\n"
			+ "FROM cal")
	...
	@Query(nativeQuery = true, value = "WITH base_assets AS (\r\n"
			...
			+ "\r\n"
			+ "    -- location based on latest record if exists\r\n"
