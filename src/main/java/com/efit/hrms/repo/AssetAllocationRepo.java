...
public interface AssetAllocationRepo extends JpaRepository<AssetAllocationVO, Long> {
	...
	@Query(nativeQuery = true, value = "SELECT DISTINCT \r\n" + "    a.employeename,\r\n" + "    a.employeecode,\r\n"
			...
			+ "  AND a.branchcode = ?4")
	List<Object[]> getAccessoriesByEmployeeCode(String employeeCode, Long orgId, String department, String branchCode);

	@Query(nativeQuery = true, value = "select * from assetallocation where orgid =?1 and (employeecode=?2 or 'ALL'=?2) and branchcode = ?3 and (?4 is null or STR_TO_DATE(createdon,'%d-%m-%Y %h:%i:%s %p') >=?4)\r\n"
			+ "and (?5 is null or STR_TO_DATE(createdon,'%d-%m-%Y %h:%i:%s %p') <= ?5 )")
	List<AssetAllocationVO> getAssetAllocationReportDetails(Long orgId, String employeeCode, String branchCode, String fromDate,
			String toDate);
